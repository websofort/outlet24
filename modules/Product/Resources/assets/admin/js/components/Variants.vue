<template>
    <div class="box-header">
        <h5>
            {{ trans("product::products.group.variants") }}
        </h5>
        ...
        <div class="d-flex">
            <span
                ...
                :class="{
                    collapsed: isCollapsedVariantsAccordion,
                }"
                data-toggle="tooltip"
                data-placement="top"
                :data-original-title="
    ...
    <div class="box-body">
        <div class="accordion-box-content form-horizontal">
                ...
                >
                    ...
                    >
                        <div class="panel panel-default">
                            <div
                                class="panel-heading"
                                @click.stop="toggleAccordion($event, variant)"
                            >
...
const { form, errors, focusField, hasAnyError, clearErrors } = useForm();
...
function changeDefaultVariant(uid) {
    ...
    if (variants[index].is_active === true) {
        ...
        defaultVariantUid.value = variants[index].uid;
        variants[index].is_default = true;

        return;
    }

    defaultVariantUid.value = defaultVariantUid.value;
}

function isActiveVariant(index) {
    return form.variants[index].is_active;
}

function changeVariantStatus(variantUid) {
    if (defaultVariantUid.value === variantUid) {
        toaster(trans("product::products.variants.disable_default_variant"), {
            type: "default",
        });
...
function removeVariantMedia(variantIndex, mediaIndex) {
    form.variants[variantIndex].media.splice(mediaIndex, 1);
}
